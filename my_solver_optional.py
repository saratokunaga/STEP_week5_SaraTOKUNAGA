...
# cities = [(x0, y0), (x1, y1), ...]
def main(cities):
    current_tour, current_whole_distance = donyoku(cities)
    best_tour, best_whole_distance = my_2opt(cities, current_tour, current_whole_distance) #(cities, tour, whole_distance)
    
    current_tour = best_tour
    current_whole_distance = best_whole_distance
    for _ in range(20):
        current_tour, current_whole_distance = yakinamashi(cities, current_tour, current_whole_distance)

        current_tour, current_whole_distance = my_2opt(cities, current_tour, current_whole_distance)

        if current_whole_distance < best_whole_distance:
            best_whole_distance = current_whole_distance
            best_tour = current_tour
    return (best_tour, best_whole_distance)
...
def donyoku(cities):
    ...
    while len(visited) <= len(cities) - 1: # visitedに最後から一個前までのインデックスが入るまで続ける、最終的にcurrent_cityは最後の都市を指す
        current_donyoku = donyoku_next(cities, current_city, visited) #計算が一回で済むように格納
...
def donyoku_next(cities, current_city, visited): # 貪欲法（各都市のxy座標の配列、現在の都市のインデックス、既訪都市のリスト）
...
def yakinamashi(cities, tour, whole_distance):

    i = random.randrange(len(tour) - 2) #0からlen(tour)-3までのランダムな整数を返す(あとでi+1を使うので最後まではつかわない)
    j = random.randrange(i + 1, len(tour) -1) 
...
if __name__ == '__main__':
    ...
    tour = main(read_input(sys.argv[1]))
    print_tour(tour)
